...
- [x] Document the mechanism: write a brief design note in the source namespace header
...
- [x] Implement video ID extraction: accept a full YouTube URL (youtube.com/watch?v=,
      youtu.be/, youtube.com/shorts/, youtube.com/embed/) or a bare video ID; return the
      canonical 11-character ID
- [x] Implement transcript fetch: GET the video watch page with hato, extract
...
- [x] Implement error surfacing: no caption tracks → `:type :tool/no-transcript`;
...
- [x] Register `youtube-transcript` in the tool registry with `:fn`, `:schema`
      (`:url-or-id` string required; `:lang` string optional), and `:description`
- [x] Write mocked tests: stub hato responses for happy path (returns transcript text
...
- [x] REPL smoke test: call `youtube-transcript` with a real public YouTube URL at the
      REPL; confirm readable transcript text (or a clean `:tool/result` error for a
      restricted video) is returned
- [x] LLM multi-hop chain smoke test: ask the assistant a question that requires
