...
- [x] Fluid full-height layout: the conversation region always fills its slot via `pad-lines` (charm's `:height` doesn't pad, and it drops trailing *empty* lines, so we pad with space-lines). The frame is now always exactly the terminal height — input pinned at the bottom, conversation taking the rest — instead of the frame (and input position) shrinking with little content and jumping after the first message.
- [x] Header: replace the slim bar with a persistent rounded-box header, split into `✦ Personal Assistant` (left) and the message-of-the-day (right) on one row. MOTD reads `:motd` from user.md front-matter (`queries/user-motd`); when unset it shows a usage tip picked once at startup (`view/random-tip` → model `:motd-fallback`, stable per session). Added `motd: ""` to the user.md template. The border is hand-drawn and dimmed (`dim-glyph`, raw `\e[2m` SGR) to match the faint MOTD — charm's `:border-fg`/styling downgrades box-drawing *edges* to ASCII, so the dim escape is emitted by hand to keep the glyphs Unicode. `viewport-height` chrome is 11 for the 3-row header.
