...
- [x] Header: replace the slim bar with a persistent framed-wordmark header (rounded box, `✦ personal assistant` + a faint message-of-the-day line). MOTD reads `:motd` from user.md front-matter (`queries/user-motd`); when unset it shows a usage tip picked once at startup (`view/random-tip` → model `:motd-fallback`, stable per session). Added `motd: ""` to the user.md template. `viewport-height` chrome 9 → 12 for the 4-row header.
