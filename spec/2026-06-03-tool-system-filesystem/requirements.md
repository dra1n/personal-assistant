...
- **Bootstrap backfill** — `pa.storage.fs/bootstrap!` gains a `create-system-templates!` step that idempotently writes a default `system/tools.md` (from a `templates/system/` resource) when absent. The baseline allowlist grants `read write` on the PA data root only — a safe default-deny posture. The `system/` directory is already created by `create-dirs!`; only the template file is missing today.
...
5. **The baseline allowlist is `read write` on the PA data root only.** Out of the box the assistant can touch only its own data directory; everything else is default-denied until the user explicitly widens `system/tools.md`. The roadmap's `assistant-data/` is conceptual shorthand for the real data root — `$PA_HOME` or `~/.config/personal-assistant` (see `pa.storage.fs/pa-home`).
